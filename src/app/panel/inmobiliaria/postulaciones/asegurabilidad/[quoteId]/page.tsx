...
function QuoteDetailContent({ quoteId }: { quoteId: string }) {
  ...
  const handleBack = () => router.push('/panel/inmobiliaria/postulaciones/asegurabilidad')

  // D-33-10: navigate to the re-quote wizard with the current quoteId as the
  // ?from= seed (Plan 33-06 wizard pre-fills from this).
  const handleReQuote = () => {
    router.push(`/panel/inmobiliaria/postulaciones/asegurabilidad/nueva?from=${quoteId}`)
  }

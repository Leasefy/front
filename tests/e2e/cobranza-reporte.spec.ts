...
//   - /panel/inmobiliaria/pagos/cobranza/reporte                (viewer + history + CSV)
//   - /panel/inmobiliaria/pagos/cobranza/reporte/thresholds     (admin editor + versions + rollback)
//   - /panel/inmobiliaria/pagos/cobranza/reporte/suscripcion    (per-user toggles)
//
...
async function assertViewerLoads(page: Page, vp: { width: number; height: number; name: string }): Promise<void> {
  ...
  await page.goto('/panel/inmobiliaria/pagos/cobranza/reporte')
...
test('thresholds PUT creates new version (v4) — D-34-04', async ({ page }) => {
  ...
  await page.goto('/panel/inmobiliaria/pagos/cobranza/reporte/thresholds')
...
test('subscription email + WhatsApp toggles PATCH independently — D-34-06', async ({ page }) => {
  ...
  await page.goto('/panel/inmobiliaria/pagos/cobranza/reporte/suscripcion')
  await page.waitForLoadState('domcontentloaded')
...
test('reporte no horizontal scroll — iPhone-14 — XR-03', async ({ page }) => {
  await assertNoScroll(page, VIEWPORTS[0], '/panel/inmobiliaria/pagos/cobranza/reporte')
})
test('reporte no horizontal scroll — iPad-Mini — XR-03', async ({ page }) => {
  await assertNoScroll(page, VIEWPORTS[1], '/panel/inmobiliaria/pagos/cobranza/reporte')
})
test('reporte no horizontal scroll — Desktop — XR-03', async ({ page }) => {
  await assertNoScroll(page, VIEWPORTS[2], '/panel/inmobiliaria/pagos/cobranza/reporte')
})

test('suscripcion no horizontal scroll — Desktop — XR-03', async ({ page }) => {
  await assertNoScroll(page, VIEWPORTS[2], '/panel/inmobiliaria/pagos/cobranza/reporte/suscripcion')
})

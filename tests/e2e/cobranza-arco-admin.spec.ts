...
// Routes:
//   - /panel/inmobiliaria/pagos/cobranza/arco             (inbox list)
//   - /panel/inmobiliaria/pagos/cobranza/arco/arco-req-001 (detail)
...
for (const viewport of VIEWPORTS) {
  test(`ARCO inbox snapshot — ${viewport.name} — XR-03`, async ({ page }) => {
    ...
    await page.goto('/panel/inmobiliaria/pagos/cobranza/arco')
...
for (const viewport of VIEWPORTS) {
  test(`ARCO detail snapshot — ${viewport.name} — XR-03`, async ({ page }) => {
    ...
    await mockPermissions(page)
    await page.goto(`/panel/inmobiliaria/pagos/cobranza/arco/${ARCO_FIXTURE_ID}`)
    await page.waitForLoadState('domcontentloaded')
    await expect(

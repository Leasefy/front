 ...
 * permissions. The load-bearing cases are the estudio/matching fallback:
 * ABSENT key → ALLOWED (so the front gate merges in any order with the
 * agent-repo pair PR), PRESENT without the action → DENIED.
 */
...
describe('isAgentModule', () => {
  it('recognizes the four agent modules', () => {
...
describe('estudio/matching — ABSENT-module = ALLOWED fallback', () => {
  ...
  it('denies when the key is present WITHOUT the action', () => {
    const perms: AgentModulePermissions = { ...BASE, estudio: [], matching: ['execute'] }
  ...
  it('allows when the key is present WITH the action', () => {
    const perms: AgentModulePermissions = { ...BASE, estudio: ['view'], matching: ['view', 'execute'] }

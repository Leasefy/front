...
describe('<ConfiguracionDeExogena>', () => {
  ...
  it('sin la migración 70 explica qué falta y no deja guardar', async () => {
    ...
    await pintar();
      ...
      expect(q('configuracion-sin-migracion')!.textContent).not.toContain('20260919002000');
    expect(q('configuracion-sin-migracion')!.textContent).not.toContain('Víctor');
    expect(q('configuracion-sin-migracion')!.textContent).toContain('todavía no está disponible');
    expect((q('guardar-configuracion') as HTMLButtonElement).disabled).toBe(true);

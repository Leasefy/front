      ...
      <div className="mx-auto max-w-[1240px] space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <House className="w-4 h-4 text-neutral-400" weight="fill" />
        ...
        <section className="rounded-2xl border border-neutral-200/80 bg-white p-5 sm:p-6">
          <div className="flex items-center justify-between gap-3 mb-5">
            ...
            <span className="inline-flex items-center gap-1.5 text-[12px] font-medium" style={{ color: '#22663F' }}>
            ...
            {STRATEGIES.map((s) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-1 text-[12px] text-neutral-400">/mes</div>
                  <div className="mt-3 flex items-center gap-2 text-[12.5px] text-neutral-600">
                    <span className="font-medium text-[#0B1220]">{s.dias}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
          ...
          <div className="space-y-6 min-w-0">
                ...
                {FACTORS.map((f, i) => {
                  ...
                  return (
                    <li key={i} className="flex items-center gap-3 py-2">
                      ...
                      <span className="text-[13px] font-medium tabular-nums" style={{ color: tone.dot }}>
                        {f.impact}
                      </span>
                    </li>
                  )
            ...
            <section className="rounded-2xl border border-neutral-200/80 bg-white overflow-hidden">
              ...
              <div className="hidden sm:grid grid-cols-[1fr_70px_130px_90px_72px_40px] gap-3 px-5 sm:px-6 py-2 border-y border-neutral-100 bg-neutral-50/60">
                {['Inmueble', 'Área', 'Canon', 'Similitud', 'Dist.', ''].map((h, i) => (
                  <span key={i} className="font-mono text-[10px] font-medium uppercase tracking-[0.08em] text-neutral-400">
                    {h}
                  </span>
                ))}
              </div>
              ...
              <ul className="divide-y divide-neutral-100">
                {COMPARABLES.map((c) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`text-[13.5px] font-medium text-[#0B1220] truncate ${isOut ? 'line-through' : ''}`}>
                        {c.dir}
                      </span>
                      <span className="text-[13px] text-neutral-600 tabular-nums">{c.area} m²</span>
                      <span className="text-[13.5px] font-medium text-[#0B1220] tabular-nums">{fmtPerMonth(c.canon)}</span>
                      ...
                      >
                        {isOut ? <ArrowRight className="w-4 h-4 rotate-180" /> : <X className="w-4 h-4" />}
                      </button>
                    </li>
                  )
                })}
          ...
          <aside className="space-y-6 lg:sticky lg:top-6">
            ...
            <section className="rounded-2xl border border-neutral-200/80 bg-white overflow-hidden">
              ...
              <div className="relative h-32 bg-gradient-to-br from-neutral-100 to-neutral-200/70 flex items-center justify-center">
                <House className="w-8 h-8 text-neutral-300" weight="duotone" />
            ...
            <section className="rounded-2xl border border-neutral-200/80 bg-white p-5">
              ...
              <div className="mt-3 space-y-3">
                <div className="flex items-center justify-between text-[13px]">
                  <span className="text-neutral-500">Carlos espera</span>
                  <span className="font-medium text-[#0B1220] tabular-nums">{fmtPerMonth(OWNER_EXPECTATION)}</span>
                </div>
                <div className="flex items-center justify-between text-[13px]">
                  <span className="text-neutral-500">Gabriela sugiere</span>
                  <span className="font-semibold tabular-nums" style={{ color: BLUE }}>{fmtPerMonth(recomendado.value)}</span>
                </div>
                {/* gap bars */}
                <div className="space-y-1.5 pt-1">
                  ...
                  <div className="h-2 rounded-full bg-neutral-100 overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${Math.round((recomendado.value / OWNER_EXPECTATION) * 100)}%`, background: BLUE }} />
                  </div>
                </div>
                <div
                  className="flex items-start gap-2 rounded-lg p-3 text-[12.5px] leading-snug"
                  style={{ background: '#FBF3E2', color: '#8A5A12' }}
                >
            ...
            <section className="rounded-2xl border border-neutral-200/80 bg-white p-5 space-y-4">
              ...
              <div className="space-y-2 pt-1">
                <Link
                  ...
                  className="group w-full inline-flex items-center justify-center gap-2 h-11 px-4 rounded-lg text-white text-sm font-medium transition-colors"

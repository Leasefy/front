...
//   - /panel/inmobiliaria/pagos/cobranza/compliance         (overview)
//   - /panel/inmobiliaria/pagos/cobranza/compliance/ley-2300
//   - /panel/inmobiliaria/pagos/cobranza/compliance/opt-out
//   - /panel/inmobiliaria/pagos/cobranza/compliance/audit
//
...
async function assertOverviewBanner(page: Page, vp: { width: number; height: number; name: string }): Promise<void> {
  ...
  await page.goto('/panel/inmobiliaria/pagos/cobranza/compliance')
  const banner = page
...
test('audit-log 4 filters present; no Revelar button adjacent to masked cedula — D-34-08 Phase 31 PII invariant', async ({ page }) => {
  ...
  await page.goto('/panel/inmobiliaria/pagos/cobranza/compliance/audit')
  await page.waitForLoadState('domcontentloaded')
  // Table renders
...
test('ley-2300 sub-page renders attempt table — COBR-UI-10', async ({ page }) => {
  ...
  await page.goto('/panel/inmobiliaria/pagos/cobranza/compliance/ley-2300')
  await expect(page.locator('table, [role="table"]').first()).toBeVisible({ timeout: 15_000 })
...
test('opt-out registry shows pending items + acknowledge button — COBR-UI-10', async ({ page }) => {
  ...
  await page.goto('/panel/inmobiliaria/pagos/cobranza/compliance/opt-out')
  await expect(page.locator('table, [role="table"]').first()).toBeVisible({ timeout: 15_000 })
  await expect(
...
test('compliance overview no horizontal scroll — iPhone-14 — XR-03', async ({ page }) => {
  await assertNoScroll(page, VIEWPORTS[0], '/panel/inmobiliaria/pagos/cobranza/compliance')
})
test('compliance overview no horizontal scroll — iPad-Mini — XR-03', async ({ page }) => {
  await assertNoScroll(page, VIEWPORTS[1], '/panel/inmobiliaria/pagos/cobranza/compliance')
})
test('compliance overview no horizontal scroll — Desktop — XR-03', async ({ page }) => {
  await assertNoScroll(page, VIEWPORTS[2], '/panel/inmobiliaria/pagos/cobranza/compliance')
})
test('compliance audit no horizontal scroll — iPhone-14 — XR-03', async ({ page }) => {
  await assertNoScroll(page, VIEWPORTS[0], '/panel/inmobiliaria/pagos/cobranza/compliance/audit')
})
test('compliance audit no horizontal scroll — Desktop — XR-03', async ({ page }) => {
  await assertNoScroll(page, VIEWPORTS[2], '/panel/inmobiliaria/pagos/cobranza/compliance/audit')
})

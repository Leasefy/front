...
function CancelModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface w-full max-w-md rounded-xl border border-border">
        {/* Header */}
...
export default function VisitasPage() {
  ...
  const columns: PlanTableColumn<Visit>[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (row) => (
        <span className="text-sm text-fg-muted">{formatDate(row.requestedDate + 'T12:00:00')}</span>
      ),
    },
    {
      key: 'requestedTime',
      ...
      sortable: true,
      render: (row) => (
        <span className="text-sm text-fg-muted">{formatTime(row.requestedTime)}</span>
      ),
    },
    {
      key: 'status',
      ...
      content: (
        <div className="space-y-3">
          <div className="flex justify-between">
            <span className="text-sm text-fg-muted">{t('landlord.visits.dateLabel')}</span>
            <span className="text-sm font-medium text-fg">{formatDate(visit.requestedDate + 'T12:00:00')}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-sm text-fg-muted">{t('landlord.visits.timeLabel')}</span>
            <span className="text-sm font-medium text-fg">{formatTime(visit.requestedTime)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-sm text-fg-muted">{t('landlord.visits.statusLabel')}</span>
            <PlanStatusBadge
          ...
          <div className="flex justify-between">
            <span className="text-sm text-fg-muted">{t('landlord.visits.propertyLabel')}</span>
    ...
    <div className="min-h-screen bg-bg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <KpiCard
            label={t('landlord.visits.totalVisits')}
          ...
          <Card className="p-5">
            ...
            <p className="text-2xl font-bold tabular-nums font-mono text-fg">{stats.requested}</p>
            <p className="text-sm text-fg-muted">{t('landlord.visits.pendingVisits')}</p>
            {stats.requested > 0 && (
          ...
          <KpiCard
            label={t('landlord.visits.confirmedToday')}
            value={String(confirmedToday)}
            icon={<CalendarCheck />}
          />

          <KpiCard
            label={t('landlord.visits.completedVisits')}
        ...
        <SegmentedControl
          size="md"
          className="mb-6"
          aria-label={t('landlord.visits.title')}
          value={tabFunnel}
          onChange={(value) => setTabFunnel(value)}
          options={tabs.map((tab) => ({
            value: tab.id as TabFunnel,
            ariaLabel: tab.label,
            label: (

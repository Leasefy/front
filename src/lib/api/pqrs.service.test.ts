...
// El apiClient de develop espera (con tope de 3s) a que el AuthProvider anuncie
// la sesión antes de cada petición. En tests nadie la anuncia, así que cada
// llamada pagaba el tope y los tests con dos llamadas vencían el timeout.
// `setAccessToken(null)` marca la sesión como resuelta — mismo patrón que
// registration-profiles.service.test.ts.
beforeEach(() => {
  setAccessToken(null);
});

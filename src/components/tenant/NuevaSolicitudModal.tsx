 ...
 *     `documentsApi.upload({ entityType:'pqrs', entityId: created.id })` — photos
...
import { documentsApi } from '@/lib/api/documents.service';
...
export function NuevaSolicitudModal({ open, onClose, onCreated, prefill }: NuevaSolicitudModalProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const created = await pqrsApi.create(input);

      // Photos attach ONLY after a real create id — never an orphaned upload.
      if (files.length > 0) {
        await Promise.all(
          files.map((file) =>
            documentsApi.upload({
              file,
              type: 'solicitud_evidencia',
              entityType: 'pqrs',
              entityId: created.id,
            }),
          ),

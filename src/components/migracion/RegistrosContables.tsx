...
import { contabilidadApi, MAX_LIMITE_DE_ASIENTOS, type CuentaPuc } from '@/lib/api/contabilidad.service';
...
export function RegistrosContables({ onIrAlPuc }: { onIrAlPuc?: () => void } = {}) {
  ...
  const cargar = useCallback(async () => {
    ...
    if (c.status === 'fulfilled') setCuentas(c.value);
      ...
      setCuentas([]);
      setError(mensajeDeContabilidad(c.reason, 'No pudimos leer el plan de cuentas.'));
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {resumen ? (
        ...
        >
          {resumen.total === 0 ? (
            <p className="text-sm text-fg-muted">Todavía no hay registros contables.</p>
          ) : (
            <p className="text-sm text-fg">
              Ya cargados:{' '}
              <span className="font-mono font-semibold tabular-nums">{resumen.total}</span>{' '}
              {resumen.total === 1 ? 'asiento' : 'asientos'}
              {resumen.desde && resumen.hasta ? (
                <span className="text-fg-muted">
                  {' '}
                  · del {formatDate(resumen.desde)} al {formatDate(resumen.hasta)}
                  {resumen.parcial ? ` (fechas de los ${MAX_LIMITE_DE_ASIENTOS} más recientes)` : ''}
                </span>
              ) : null}
            </p>
          )}
      ...
      {cuentas.length === 0 ? (
        <section className="rounded-lg border border-warning bg-warning-soft p-5" data-testid="contables-sin-puc">
          <div className="flex items-start gap-2">
            <Warning className="mt-0.5 h-4 w-4 shrink-0 text-warning" />
            <div>
              <h2 className="font-medium text-fg">Primero el plan de cuentas</h2>
              <p className="mt-0.5 text-sm text-fg-muted">
                Un asiento se imputa a cuentas, y todavía no hay ninguna que reciba movimientos.
                Cargá el plan en el paso 4 y volvé.
              </p>
            </div>
          </div>
          {onIrAlPuc ? (

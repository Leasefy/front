 ...
 *
 * Vista ERP del egreso neto al propietario con la fórmula completa que el motor
 * (M1 — back-main) calculará de forma autoritativa:
 *   canon recibido − comisión admin − IVA sobre comisión − descuentos = neto
 *
...
export interface EgresoNeto {
  ...
  neto: number;               // COP — neto a pagar = canon − comisión − IVA − descuentos
  cuentaDestino?: string;     // banco + número (enmascarado)
  estado: EgresoEstado;
  comprobanteUrl?: string;    // comprobante de egreso (PDF), cuando procesado
}

/** Cálculo del neto a partir de las partes (mirror de lo que hará M1). */
export function calcularNeto(parts: {
  ...
  return parts.canonRecibido - parts.comisionAdmin - parts.ivaComision - parts.descuentos;
}

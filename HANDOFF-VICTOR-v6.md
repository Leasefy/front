...
## 14. QA highs H-21..H-26 (agent) — branch `fix/qa-agent-highs-h21-h26` (LOCAL, Víctor pushea)

Los **8 altos agent-side** del QA audit (`mvp/claudedocs/v6-full-qa-audit-2026-06-01.md`) = H-21..H-28. **5 arreglados** (H-22..H-26, commit `9666e42`), **3 diferidos con razón** (H-21, H-27, H-28 — ver filas abajo: no son fixes ciegos, requieren decisión de diseño/roadmap/legal). Rama nueva `fix/qa-agent-highs-h21-h26` apilada sobre `restructure/per-agent-organization` (ya incluye C-3/C-4/C-5/H-1/H-5-7). **Sin pushear** (cuenta sin write access → Víctor pushea). `tsc --noEmit` limpio; suite completa: **+10 tests nuevos verdes, 0 regresiones** (las 27 fallas restantes son idénticas al base — pre-existentes: OpenAPI snapshot/version, voice-conductor LLM API-key-gated, wompi sandbox, integration tests con DB). Todo lo arreglado es lógica de **enforcement/redacción** — NO toca copy deudor-facing, así que no requiere abogado (la frontera de confianza es el guardrail, no el prompt).

| ID | Qué se hizo | Archivo(s) | Tests | Nota compliance |
|---|---|---|---|---|
| **H-22** | El dialer ramifica en `event.data.channel`: un evento ruteado a `whatsapp` (el priorizador hace swap cuando el cap semanal de voz se agota) ahora **skip + audit** (`dialer.call_skipped` / `channel_whatsapp_not_dialed`) en vez de llamar por voz igual. NO inventa un envío de WhatsApp desde el dialer (`composeWhatsAppTool` requiere callId/summary/agencyPolicy que el evento no trae). | `src/inngest/functions/autonomous-dialer-workflow.ts` | +2 (`autonomous-dialer-workflow.test.ts`) | **Compliance-positivo:** cierra una posible brecha del cap semanal de voz Ley 2300. |
| **H-23** | Un envío real de WhatsApp (`status==='sent'`, NO `stubbed`) ahora registra `recordContactAttempt(channel:'whatsapp')` en la capa única `sendWhatsAppTool` (cubre el path autónomo `follow-up-workflow`). No-fatal: traga P2002 (mismo-día) y prisma-null. | `src/mastra/tools/send-whatsapp.ts` | +4 (`send-whatsapp.test.ts`) | Ley 2300: antes WhatsApp no contaba en el cap diario-total ni semanal-por-canal. **Pendiente (seguimiento):** otros senders NO pasan por el tool y siguen sin registrar — `closer.ts` (WA mid-call, colisión P2002 mismo-día = correcto, swallow), `cartera-daily-report-cron.ts`, `agency-cobranza-interventions.ts` (manual), y `whatsapp-webhook.ts` PAY_NOW (inbound-triggered → contarlo como contacto outbound es **decisión de producto/legal**). |
| **H-24** | `maxCanonCop` por aseguradora ahora se impone en `dispatchEffectiveQuotes`: las carriers sobre-cap se excluyen del set `enabled` (todas las ramas lo heredan) y se reportan como `failures[]` estructuradas. Cap `null` = sin cap; boundary `>` estricto. | `src/mastra/agents/cotizador/quote-orchestrator.ts` | +3 (`quote-orchestrator-dispatch.test.ts`) | Sin ángulo Ley 2300 (es elegibilidad de suscripción). |
| **H-25** | Los 3 sitios de salida del trace del cotizador (respuesta HTTP sync, evento sync, evento stream) ahora emiten el trace **redactado** (`traceForPersist` / `streamTraceForPersist`), no el crudo — enforcea el invariante que el propio archivo ya documenta (L112-115/L537). | `src/server/routes/cotizador.ts` | cubierto por shape-test existente + chokepoint guard (verde) | Habeas Data/Ley 1581: cerraba una fuga de PII (nombres/teléfonos/direcciones) al backend B2B aunque la copia en DB sí se limpiaba. **Verificar (seguimiento):** el trace que viaja al cliente vía el evento SSE `agent.final_verdict` en vivo (dentro de `runExtendedQuote`) — confirmar que también está redactado; este lote sólo tocó los 3 sitios de persist/emit de la ruta. |
| **H-26** | El header `X-Cotizador-Consent` ahora exige un id de autorización no-trivial (`/^[A-Za-z0-9._-]{16,}$/`) → `signed-x` se rechaza con 400. + nota de contrato explícita: es un gate de **presencia+formato**, NO prueba criptográfica (modelo recaudador: la inmobiliaria tiene la autorización firmada). | `src/server/routes/cotizador.ts` | +1 (`cotizador.test.ts`) + 1 fixture stale arreglado | **Decisión legal pendiente:** si la SIC exige prueba server-side, añadir una tabla `consent_record` (cedula_hash + scope + signed_at) tenant-scoped y validar ahí — gated por counsel; NO construir especulativamente (inventar consentimiento que el sistema nunca recolectó sería peor). |
| **H-21** | **DIFERIDO — decisión de roadmap.** `cartera-cadence-cron` computa+persiste `cadence_contacts` pero NINGÚN cron/función lo lee por `planned_for` para despachar; todos los lectores son rutas UI de solo-lectura. El path de marcado vivo es `pre-call-workflow` (su propio `prioritizeCartera()`). Cablear un dispatcher que lea `cadence_contacts` y emita `cobranza/call.scheduled` **arriesgaría doble-contacto** con `pre-call-workflow` → violación Ley 2300. **Decisión necesaria (roadmap 17.8):** (a) `cadence_contacts` es display-only y `pre-call-workflow` es el dispatcher único intencional (entonces documentarlo + reconciliar el plan mostrado), o (b) cadence_contacts debe despachar y `pre-call-workflow` se retira/dedup. NO se tocó código. | — | — | Riesgo Ley 2300 si se cablea mal (doble contacto). |
| **H-27** | **DIFERIDO — el fix del audit (renombrar evento) es INSUFICIENTE/incorrecto.** Sí hay un mismatch de nombre: emisor `mastra/tools/schedule-follow-up.ts:39` emite `cobranza/follow-up.scheduled` (con guión); consumidor `inngest/functions/follow-up-workflow.ts:90,366` dispara en `cobranza/followup.scheduled` (sin guión). PERO los **payloads son incompatibles**: el tool envía `{debtorId, callId, dueDate, reason}` (sin `tenantId`/`finalState`/`summaryJson`) y `follow-up-workflow` EXIGE `tenantId`+`finalState`+`summaryJson` (post-call) e **ignora `dueDate`**. Son eventos semánticamente distintos: el tool agenda re-enganche FUTURO en `dueDate`; el workflow procesa el follow-up post-llamada AHORA. Renombrar solo rutearía eventos sub-especificados → `withTenantScope(undefined)` + `finalState` vacío → fallo silencioso. **Decisión de diseño necesaria:** (a) construir un handler propio de re-enganche diferido (sleep-until `dueDate` → emitir contacto con gates Ley 2300), o (b) enriquecer el payload del tool + hacer que el workflow tolere una variante "scheduled" sin contexto de llamada. NO se tocó código. | — | — | El tool devuelve `scheduled=true` (monitoreo en verde) pero el deudor nunca se re-contacta por ese path. |
| **H-28** | **DIFERIDO — wire-up claro pero activa generación de carta pre-jurídica (gated por counsel).** `cobranza/cartera.stage.transitioned` está registrado como trigger en `legal-escalation-workflow.ts:217` pero NINGÚN `inngest.send` lo emite → la rama S3 pre-jurídica nunca corre. **Patch listo para Víctor:** en `cartera-cadence-cron.ts`, hacer que el step `compute-transitions` devuelva además las transiciones a `S3` (`{debtorId, fromStage}`), y en un **step SEPARADO** (`emit-s3-transitions`, NO dentro del tx — `carteraStageTransition.create` es append-only, un retry dentro del tx duplicaría filas) emitir `inngest.send({ name: 'cobranza/cartera.stage.transitioned', data: { tenantId, debtorId, toStage: 'S3' } })` por cada una. El workflow filtra `toStage==='S3'`, es idempotente (`buildPreJudicialLetter` con `idempotentReuse`) y solo CONSTRUYE el artefacto PDF + audit (no auto-envía al deudor). **Pre-requisito legal:** la plantilla `buildPreJudicialLetter` es copy deudor-facing → revisión de abogado antes de activarla en prod (regla del proyecto). + test en `cartera-cadence-cron.test.ts`. NO se tocó código (activación = decisión producto/legal). | — | — | Carta pre-jurídica = artefacto legal → counsel antes de prod. |

 ...
 * Ruta: /panel/inmobiliaria/tesoreria/facturas/nueva
 *
      ...
      <section className="rounded-xl border border-border bg-card p-5 lg:p-6 max-w-5xl">
        {agency?.id ? (
          <FacturaProveedorIACapture
            agencyId={agency.id}
            onRegistrada={(bill) => router.push(`/panel/inmobiliaria/tesoreria/ap/${bill.id}`)}
            onCancel={() => router.push('/panel/inmobiliaria/tesoreria')}
          />
        ) : (

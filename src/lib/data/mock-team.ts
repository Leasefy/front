/**
 * Mock team data for collaboration features
 * @module lib/data/mock-team
 */
...
export function getActiveTeamMembers(): TeamMember[] {
  return MOCK_TEAM_MEMBERS.filter((m) => m.status === 'accepted');
}

export function getPendingInvites(): TeamMember[] {
  return MOCK_TEAM_MEMBERS.filter((m) => m.status === 'pending');
}
